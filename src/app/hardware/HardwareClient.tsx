...
export default function HardwareClient() {
  ...
  return (
    <>
      ...
      {/* Hardware Categories */}
      <section className="py-24 bg-[var(--surface)] border-b border-[var(--border-color)]" id="products">
            ...
            {hardwareCategories.map((category) => {
              ...
              return (
                <StaggerItem key={category.id}>
                  <div id={category.id} className="group h-full p-8 bg-background border border-[var(--border-color)] hover:border-foreground transition-colors flex flex-col hover:bg-foreground hover:text-background scroll-mt-32">
                    <div className="mb-8">
                      <Icon className="w-8 h-8 text-muted-foreground group-hover:text-background transition-colors" />
                    </div>
                    <h3 className="text-xl font-bold text-foreground group-hover:text-background mb-4 transition-colors tracking-tight">{category.title}</h3>
                    <p className="text-[15px] text-muted-foreground max-w-prose group-hover:text-muted leading-[1.6] mb-8 flex-grow transition-colors">
                      ...
                      <ul className="space-y-4 mb-8">
                        {category.features.slice(0, 4).map((f) => (
                          <li key={f} className="flex items-start gap-4">
                            <CheckCircle2 className="w-4 h-4 text-muted-foreground group-hover:text-background mt-0.5 flex-shrink-0 transition-colors" />
                            <span className="text-[15px] text-foreground group-hover:text-background transition-colors">{f}</span>
                          </li>
                        ))}
                      </ul>
                    )}
      ...
      {/* Request Quote Form */}
      <section className="py-24 bg-background border-b border-[var(--border-color)]" id="quote">
            ...
            <form onSubmit={handleSubmit} className="border-t border-[var(--border-color)] pt-12" noValidate>
              <div className="grid sm:grid-cols-2 gap-8 mb-8">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-transparent border-b border-[var(--border-color)] text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-foreground transition-colors pb-3 text-base rounded-none"
              ...
              <div className="grid sm:grid-cols-2 gap-8 mb-8">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-transparent border-b border-[var(--border-color)] text-foreground focus:outline-none focus:border-foreground transition-colors pb-3 text-base appearance-none cursor-pointer rounded-none [&>option]:bg-background [&>option]:text-foreground"
                    required
                  >
              ...
              <div className="mb-12">
                ...
                <textarea
                  ...
                  className="w-full bg-transparent border-b border-[var(--border-color)] text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-foreground transition-colors pb-3 text-base resize-none rounded-none"
                  placeholder="Required specs, brands preferred, delivery timeline, etc."
                />
              </div>
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-3 px-8 py-4 bg-foreground text-background font-semibold text-sm uppercase tracking-[0.1em] hover:opacity-90 transition-opacity rounded-none"
              >
      ...
      <section className="py-20 bg-[var(--surface)] text-foreground text-center px-4 relative overflow-hidden border-b border-[var(--border-color)]">
        ...
        <ScrollAnimationWrapper className="relative z-10 max-w-5xl mx-auto">
          ...
          <div className="flex justify-center">
            ...
            >
              Talk to an Expert <ArrowRight className="w-5 h-5 group-hover:translate-x-2 transition-transform duration-300" />
            </Link>
          </div>
        </ScrollAnimationWrapper>
      </section>
