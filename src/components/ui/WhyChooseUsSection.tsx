        ...
        <StaggerChildren className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {differentiators.map((item, index) => {
            ...
            return (
              <StaggerItem
                key={item.id}
                className={`group relative bg-background border border-[var(--border-color)] hover:border-[var(--accent)] transition-all duration-500 overflow-hidden ${
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <div className={`mb-6 flex items-center justify-center rounded-xl bg-[var(--surface)] border border-[var(--border-color)] group-hover:bg-background transition-colors ${
                    isFeatured ? "w-16 h-16" : "w-12 h-12"
                  }`}>
                    <Icon className={`text-[var(--accent)] ${isFeatured ? "w-8 h-8" : "w-6 h-6"}`} />
                  </div>
